...
async def send_welcome(message: types.Message):
    ...
    caption = (
        ...
        f"Это официальный бот <b>REY AGENCY</b> для приёма заявок.\n\n"
        f"▸ <b>Оператор</b> — модерация чата на стримах, общение с аудиторией\n"
        f"▸ <b>Скаут</b> — поиск и привлечение новых участников\n\n"
        f"⬇️ Выбери роль, на которую хочешь подать заявку"
    )
    try:
...
async def callbacks(callback: types.CallbackQuery):
    ...
    if callback.data.startswith("reject_app:"):
        ...
        admin = callback.from_user
        admin_name = f"@{admin.username}" if admin.username else (admin.first_name or "Админ")
        admin_chat_url = f"tg://user?id={admin.id}"
        reject_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💬 Написать администратору", url=admin_chat_url)]
        ])
        try:
            await bot.send_message(
                app["user_id"],
                "🌑 Твоя заявка на скаута отклонена.\n\n"
                "Если хочешь уточнить причину — напиши администратору напрямую.",
                parse_mode=None,
    ...
    if callback.data.startswith("iq_"):
        ...
        if state["question"] < len(IQ_QUESTIONS):
            ...
            if app_type == "operator":
                ...
                await callback.message.answer(
                    "🌑 <b>Укажи свой возраст или дату рождения</b>\n\n"
