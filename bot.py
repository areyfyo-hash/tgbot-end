...
async def send_welcome(message: types.Message):
    ...
    caption = (
        f"🖤 <b>{name}</b>, добро пожаловать в <b>REY AGENCY</b>.\n\n"
        f"━━━━━━━━━━━━━━━━━\n"
...
async def callbacks(callback: types.CallbackQuery):
    ...
    if callback.data.startswith("chat_with_admin:"):
        ...
        if user_id not in operator_in_chat:
            operator_in_chat[user_id] = set()
        operator_in_chat[user_id].add(admin_id)
        admin_chat_state[admin_id] = {"user_id": user_id, "app_id": app_id}
        try:
            await bot.send_message(
                admin_id,
                f"💬 <b>Пользователь хочет уточнить причину отказа</b>\n\n"
                f"👤 {full_name} ({username_str})\n"
                f"Заявка #{app_id}\n\n"
                f"Пишите — сообщения будут доходить ему прямо в боте.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔴 Закрыть чат", callback_data=f"close_chat:{app_id}")]
                ])
            )
            ...
            operator_in_chat.pop(user_id, None)
            admin_chat_state.pop(admin_id, None)
            await callback.answer("Не удалось открыть чат. Попробуй позже.", show_alert=True)
            return
        try:
            await callback.answer()
        except Exception:
